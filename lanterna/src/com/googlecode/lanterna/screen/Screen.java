...
public class Screen {
	...
	public Screen(Terminal terminal, int terminalWidth, int terminalHeight) {
		...
		this.paddingCharacter =
				new ScreenCharacter('X', Terminal.Color.GREEN,
	...
	public void setCursorPosition(int column, int row) {
		synchronized (mutex) {
			if (column >= 0 && column < terminalSize.getColumns() && row >= 0
	...
	public void setPaddingCharacter(char character,
			Terminal.Color foregroundColor, Terminal.Color backgroundColor,
			ScreenCharacterStyle... style) {

		this.paddingCharacter =
				new ScreenCharacter(character, foregroundColor,
						backgroundColor, new HashSet<ScreenCharacterStyle>(
								Arrays.asList(style)));
	}
	...
	public void startScreen() {
		...
		if (cursorPosition != null) {
			...
			terminal.moveCursor(cursorPosition.getColumn(),
	...
	public void putString(int x, int y, String string,
		...
		Set<ScreenCharacterStyle> drawStyle =
				EnumSet.noneOf(ScreenCharacterStyle.class);
	...
	public void putString(int x, int y, String string,
		...
		for (int i = 0; i < string.length(); i++) {
			putCharacter(x + i, y, new ScreenCharacter(string.charAt(i),
					foregroundColor, backgroundColor, styles));
		}
	}

	void putCharacter(int x, int y, ScreenCharacter character) {
		synchronized (mutex) {
			if (y < 0 || y >= backbuffer.length || x < 0
					|| x >= backbuffer[0].length) {
				return;
			}
	...
	public void refresh() {
		...
		synchronized (mutex) {
			...
			Map<TerminalPosition, ScreenCharacter> updateMap =
					new TreeMap<TerminalPosition, ScreenCharacter>(
			...
			for (TerminalPosition nextUpdate : updateMap.keySet()) {
				if (previousPoint == null
						...
						|| previousPoint.getColumn() + 1 != nextUpdate
								.getColumn()) {
					terminalWriter.setCursorPosition(nextUpdate.getColumn(),
							nextUpdate.getRow());
				}
			...
			if (cursorPosition != null) {
				...
				terminalWriter.setCursorPosition(cursorPosition.getColumn(),
						cursorPosition.getRow());
			} else {
	...
	private void resizeScreenIfNeeded() {
		...
		ScreenCharacter[][] newVisibleScreen =
				new ScreenCharacter[height][width];
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (backbuffer.length > 0 && x < backbuffer[0].length
						&& y < backbuffer.length) {
				...
				if (visibleScreen.length > 0 && x < visibleScreen[0].length
						&& y < visibleScreen.length) {
					newVisibleScreen[y][x] = visibleScreen[y][x];
				} else {
					newVisibleScreen[y][x] =
							new ScreenCharacter(paddingCharacter);
				}
			}
		}
	...
	private static class ScreenPointComparator implements
			Comparator<TerminalPosition> {
		@Override
		public int compare(TerminalPosition o1, TerminalPosition o2) {
			if (o1.getRow() == o2.getRow()) {
				if (o1.getColumn() == o2.getColumn()) {
					...
					return new Integer(o1.getColumn())
							.compareTo(o2.getColumn());
				}
			} else {
		...
		void writeCharacter(ScreenCharacter character) {
			if (currentlyIsBlinking != character.isBlinking()) {
				if (character.isBlinking()) {
					...
					terminal.applyBackgroundColor(character
							.getBackgroundColor());
